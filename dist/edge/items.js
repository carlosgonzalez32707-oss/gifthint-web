...
import {
  getWishlists        as _getWishlists,
  getLastUsedWishlist as _getLastUsedWishlist,
  setLastUsedWishlist as _setLastUsedWishlist,
} from './wishlists.js'

// Re-export so popup.js only needs to import from items.js
export { _flushSaveQueue      as flushSaveQueue      }
export { _getWishlists        as getWishlists        }
export { _getLastUsedWishlist as getLastUsedWishlist }
export { _setLastUsedWishlist as setLastUsedWishlist }
 ...
 *   - wishlistId = null → item saved without a list assignment (orphan)
 *
 * @param {{ title, price, currency, image_url, source_url, retailer }} productData
 * @param {string} userId
 * @param {string} token
 * @param {string|null} [wishlistId]  UUID of the target wishlist (from popup dropdown)
 */
export async function saveItem(productData, userId, token, wishlistId = null) {
